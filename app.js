...
function updateWantsNeedsSavings() {
    ...
    if (needsDiv) {
        needsDiv.innerHTML = "";
        needsDiv.appendChild(
            createLine("Needs", needs, needsPct, targets.needs),
        );
    }
    if (wantsDiv) {
        wantsDiv.innerHTML = "";
        wantsDiv.appendChild(
            createLine("Wants", wants, wantsPct, targets.wants),
        );
    }
    if (savingsDiv) {
...
function saveTasks() {
    const inputs = articles.map((article) =>
        Array.from(article.querySelectorAll("input")).map(
            (input) => input.value,
        ),
    );
